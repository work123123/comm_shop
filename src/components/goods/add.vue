      ...
      <el-tabs v-model="activeIndex" tab-position='left' :before-leave="beforeTableave" @tab-click="tabClicked">
        <el-tab-pane label="基本参数" name="0">
          ...
          <el-form-item label="商品分类" prop="">
            <el-cascader expand-trigger="hover" :options="catelist" v-model="addForm.goods_cat" :props="cateProps"
            @change="handleChange">
            </el-cascader>
          </el-form-item>
        </el-tab-pane>
  ...
  methods: {
    ...
    add () {
      this.$refs.addFormRef.validate(async valid => {
        ...
        // 修改数据类型符合api,使用深拷贝 lodash cloneDeep(obj)
        // 因为 catelist 跟标签关联v-model 必须是数组 才需要拷贝一个出来
